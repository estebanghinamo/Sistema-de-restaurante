...
ADD proposito NVARCHAR(20) NOT NULL DEFAULT '';
select * from dbo.contenidos
-- 2. Actualizar con los valores específicos para cada contenido
UPDATE dbo.contenidos SET proposito = 'Proposito1' WHERE nro_contenido = 1;
UPDATE dbo.contenidos SET proposito = 'Proposito2' WHERE nro_contenido = 2;
UPDATE dbo.contenidos SET proposito = 'Proposito3' WHERE nro_contenido = 3;
UPDATE dbo.contenidos SET proposito = 'Proposito4' WHERE nro_contenido = 4;*/
...
ADD fecha_expiracion Date NULL;
...
BEGIN
    ...
    DECLARE @nro_sucursal INT =
        CAST(SUBSTRING(@cod_sucursal_restaurante,
...
BEGIN TRY
    ...
    IF NOT EXISTS (
        ...
        THROW 51014, 'El turno no existe o no está habilitado.', 1;
    ...
    IF (@capacidad_total - @ocupados) < @cant_personas
        THROW 51020, 'No hay cupo disponible.', 1;
    ...
    FROM dbo.clientes c
    WHERE c.nro_cliente = @cliente_id;
    ...
    SELECT 
        cod_reserva = (SELECT TOP 1 cod_reserva FROM @t),
        puntos = @puntos,
        categoria = @categoria,
...
BEGIN CATCH
    ...
    IF ERROR_NUMBER() IN (2627,2601)
       AND CHARINDEX('AK_clientes_correo', ERROR_MESSAGE()) > 0
