...
/* si ya existen contenidos en ristorino debo borrarlos xq el sp no actualiza comprueba si estan y lo borra
...
BEGIN
    ...
    DECLARE @puntos INT = CAST(JSON_VALUE(@json, '$.puntos') AS INT);
    DECLARE @categoria NVARCHAR(50) = JSON_VALUE(@json, '$.categoria');
    DECLARE @fecha_expiracion DATE = CAST(JSON_VALUE(@json, '$.fechaExpiracion') AS DATE);

    ------------------------------------------------------------
    -- Restaurante desde codSucursalRestaurante
    ------------------------------------------------------------
    DECLARE @cod_sucursal_restaurante NVARCHAR(20) = JSON_VALUE(@json, '$.codSucursalRestaurante');
    DECLARE @nro_restaurante INT =
        CAST(LEFT(@cod_sucursal_restaurante,
...
BEGIN TRY
    ...
    IF (@cant_adultos + @cant_menores) <= 0
        THROW 51012, 'La cantidad de comensales debe ser mayor a 0.', 1;

    ------------------------------------------------------------
    -- Construir cod_reserva_sucursal
    ------------------------------------------------------------
    DECLARE @cod_reserva_sucursal NVARCHAR(50);
    SET @cod_reserva_sucursal =
        CONCAT(@cod_reserva_restaurante, '-', CONVERT(NVARCHAR(10), @nro_sucursal));
    ...
    IF @nro_cliente IS NULL
        THROW 51013, 'El cliente no existe o está deshabilitado.', 1;

    ------------------------------------------------------------
    -- Calcular nro_reserva incremental
    ------------------------------------------------------------
    ...
    ------------------------------------------------------------
    -- Insertar reserva con fidelización
    ------------------------------------------------------------
